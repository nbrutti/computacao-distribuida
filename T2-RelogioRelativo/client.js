...
// Inicializa o vetor de relógios
const vector = new VectorClock(params.neighboors, params.port);
const queue = [];
      ...
      reader.question('Port: ', port => {
        reader.question('Type a message: ', message => {
          ...
          if ((hostname === '' && port === '') && message === '') 
            return inputMessage();
          queue.push(packet.create(hostname, port, message, vector));
          console.log("\nFila de saída: ");
          console.table(queue);
          inputMessage();
        });
